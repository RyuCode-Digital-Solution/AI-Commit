...
class AICommit:
    ...
    def generate_commit_message(self, diff: str) -> Optional[str]:
        ...
        prompt = f"""Based on the following git diff, generate a clear and descriptive commit message following the conventional commits format.

Git diff:
{diff[:3000]}

Create the commit message in this format:
<type>(<scope>): <subject>

<body (optional)>

Types can be: feat, fix, docs, style, refactor, test, chore
Use English, keep it short and clear.
